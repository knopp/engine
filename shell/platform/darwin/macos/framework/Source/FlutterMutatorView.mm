...
- (BOOL)isFlipped {
  // isFliped also affects sublayerTransform. If false the layer is rotated
...
NSMutableArray* ClipPathsFromMutations(CGRect master_clip, const MutationVector& mutations) {
    ...
    switch (mutation.type) {
      ...
      case kFlutterPlatformViewMutationTypeClipRect: {
        ...
        if (!AffineTransformIsOnlyScaleOrTranslate(affineTransform)) {
          rounded_rects.push_back({FlutterRoundedRect{
                                       mutation.clip_rect,
                                       FlutterSize{0, 0},
                                       FlutterSize{0, 0},
                                       FlutterSize{0, 0},
...
- (void)updatePlatformViewWithBounds:(CGRect)untransformedBounds
  ...
  CGAffineTransform affineTransform = CATransform3DGetAffineTransform(transform);
  affineTransform.tx -= transformedBounds.origin.x;
  affineTransform.ty -= transformedBounds.origin.y;
...
- (void)applyFlutterLayer:(const FlutterLayer*)layer {
  ...
  NSMutableArray* paths = ClipPathsFromMutations(masterClip, mutations);
